...
- **Revert pins its vault into every request.** The client resolves the ambient vault when it *builds* a request, which happens after the hop off the main actor, so a re-check on the model side after each await still leaves a one-hop window (the adversarial verifier reproduced a write of vault A's content into vault B). The four calls revert makes therefore take an explicit `inVault:`, captured once at the start. Cross-vault inbox rows remain a known gap: MCP events carry no vault tag, so an agent commit from another vault opens against the active one. Tagging MCP events with their vault is an engine follow-up.
