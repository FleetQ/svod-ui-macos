...
## Usability fix (2026-07-02, v0.2.1)
User couldn't edit / see token / copy prompt after creating an agent — actions existed but ONLY in a right-click contextMenu (undiscoverable; `swipeActions` is a no-op on macOS Forms). Fixed in `AgentsSettingsView.swift` (commit `6c3662f`, shipped in app v0.2.1): visible per-row ⋯ Menu (Edit/Copy token/Copy connection/Copy prompt/Revoke) + `.contentShape(Rectangle())`; edit sheet now SHOWS the existing token (readToken from the `file:` ref) with copy; new-agent drafts prefill `AgentDraft.defaultPrompt` (starter system prompt) instead of an empty field. Verified live: engine round-trips `prompt` fine (POST→GET→DELETE probe on :7619) — the empty prompt was user-side, field started blank. NB: engine PUT re-resolves tokenRef on registry reload — unresolvable ref (e.g. missing env:) → 500.

## MCP partial-edit tool (engine, 2026-07-02)
