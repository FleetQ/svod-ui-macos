...
- **SvodMcpServer.kt** — logs session init/close + unknown-session 404s (they were invisible). mcp.* tests green. DEPLOYED 2026-07-03 23:39 (installDist+kickstart, ready in 52s) and LIVE-VALIDATED end-to-end: Desktop's bridge POSTed its pre-restart session id → engine logged `unknown MCP session … → 404` → bridge re-initialized in 0.7s (`MCP session … initialized for agent 'claude-desktop'`). The exact scenario that wedged mcp-remote for hours now self-heals in <1s, observable on both sides.
- `lattice` entry ALSO migrated (delegated via Harbormaster to ai-browser agent): bridge at `ai-browser/apps/desktop-macos/Scripts/lattice-mcp-bridge.mjs` (commit 8744f99, pushed) — NB the lattice gateway signals a stale session as HTTP **400 with id:null body**, not 404; its bridge handles that variant. Desktop config migrated for both, Desktop restarted, all repos pushed. App v0.2.8 (build 10) released with the EngineModel reconnect fix.
- Leftover: vault note `messy/scratch-mcp-debug.md` (repro scratch) — delete when convenient.
